...
public record HydrusHeaderValue(
    ...
    [property: JsonPropertyName("approved")] string? Approved = null,
    [property: JsonPropertyName("reason")] string? Reason = null);

public record HydrusHeadersResponse(
    [property: JsonPropertyName("network_context")] HydrusNetworkContext? NetworkContext,
    [property: JsonPropertyName("headers")] Dictionary<string, HydrusHeaderValue> Headers);

public record HydrusNetworkContext(
    [property: JsonPropertyName("type")] int Type,
    [property: JsonPropertyName("data")] string Data);

public record HydrusSetHeadersRequest(
    string? Domain,
...
public sealed class HydrusHeaderManager(HttpClient httpClient)
{
    ...
    public async Task SetHeadersAsync(Dictionary<string, HydrusHeaderValue> headers, string? domain = null)
    {
        var request = new HydrusSetHeadersRequest(domain, headers);
        var response = await httpClient.PostAsJsonAsync("manage_headers/set_headers", request);
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task SetUserAgentAsync(string userAgent)
    {
        ...
        var response = await httpClient.PostAsJsonAsync("manage_headers/set_user_agent", request);
        response.EnsureSuccessStatusCode();
    }
}